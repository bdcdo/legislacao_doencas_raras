"""
Scraper ANVISA usando biblioteca raspe (com bypass automático de Cloudflare)

Este script utiliza a biblioteca raspe que implementa Playwright + stealth
para bypass automático da proteção Cloudflare do portal ANVISALegis.
...
def main():
    print("=" * 60)
    print("SCRAPER ANVISA - Usando biblioteca raspe")
    print("=" * 60)
    ...
    for i, termo in enumerate(TERMOS_BUSCA, 1):
        ...
        scraper = raspe.anvisa(headless=True, debug=False)
    ...
    output_file = f"anvisa_{timestamp}.csv"
